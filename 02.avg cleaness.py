...
def clean_reviews(text):
    # Basic cleanup: remove lines with certain keywords (e.g., 'Read more', 'like this review')
    unwanted_patterns = [r'Watched by', r'Rewatched by',r'Forgotten username or password?', r'Press Tab to complete, Enter to create', 
                         r'A moderator has locked this field.',r'© Letterboxd Limited. Made by fans in Aotearoa New Zealand.',r'Film data from TMDb.', 
                         r'Mobile site.', r'This site is protected by reCAPTCHA and the Google privacy policy and terms of service apply.\n', 
                         r'A moderator has locked spoilers for this review', r'Your diary date (if set) and watched status for this film will remain publicly visible if you change the privacy level of this entry. Add members to your Close Friends from their profile. Set your account default in Settings.']
    
    for pattern in unwanted_patterns:
        text = re.sub(pattern, ' ', text, flags=re.IGNORECASE)
    
    # Optionally, filter short lines or lines without much content
    text = "\n".join([line for line in text.splitlines() if len(line) > 3])
    return text
...
print(cleaned_reviews)  # or save to file as needed
